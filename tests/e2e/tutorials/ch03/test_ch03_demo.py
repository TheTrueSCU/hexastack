"""Feature demo recordings for Tutorial Chapter 3: Role-Based Access Control & JWT Auth."""
...
def test_todo_ch03_cli_demo() -> None:
    """Record Chapter 3 JWT Auth & RBAC in terminal video (replaying Chapters 1-2 silently)."""
    app = _build_cli_app()
    repo_root = Path.cwd()
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            ...
            # Replay steps 1-2 silently
            step_ch01_scaffold_minimal(narrator, record=False)
            step_ch02_configure_sqlite(narrator, record=False)

            # Record step 3
            step_ch03_configure_jwt_auth(narrator, record=True)

            narrator.step("JWT authentication and RBAC domain policies enforced")
