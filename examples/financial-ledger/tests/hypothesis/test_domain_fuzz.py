...
@given(
    ...
    credit_diff=st.decimals(
        min_value=Decimal("0.01"), max_value=Decimal("100.00"), places=2
    ),
)
def test_fuzz_unbalanced_transactions_always_fail(
    debit_amount: Decimal, credit_diff: Decimal
):
    ...
    entries = [
        TransactionEntry(
            account_id="acc-1", direction=EntryDirection.DEBIT, amount=debit_amount
        ),
        TransactionEntry(
            account_id="acc-2", direction=EntryDirection.CREDIT, amount=credit_amount
        ),
    ]

    with pytest.raises(UnbalancedTransactionError):
        JournalTransaction(
            reference="FUZZ-UNBALANCED", description="Unbalanced", entries=entries
        )
